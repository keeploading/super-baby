# pm-architect-workflow（产品经理 + 架构师 文档协作模板）

一套可复制的 Claude Code 协作能力：两个专家 agent 在"需求文档"和"功能设计文档"两个阶段互相主笔/评审，最终把结论交给你拍板。
...
| 生成《功能设计文档》 | architect | product-manager | 双方往返直至一致 → 交你**决策** |
...
脚本会：
1. 把两个 agent 复制到 `<目标>/.claude/agents/`
...
Claude 会自动按上表编排两个 agent，每轮结束都会告诉你进展、是否达成一致、以及需要你确认/决策的事项。
